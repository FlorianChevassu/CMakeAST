...
BasicNode::PolymorphicNode TokenToNode<cmake::language::ElementType::Arguments>::Build(const cmake::language::Token& i_token)
{
  ...
  // The Arguments token does not take into account the parenthesis. Add them to the node range.
  auto finalRange = this->ConvertRange(i_token.range);
  finalRange.begin.column -= 1;
  finalRange.end.column += 1;
